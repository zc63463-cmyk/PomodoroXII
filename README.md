...
> **当前状态：** 后端 Phase A–C 已完成；**前端（Phase F）尚未开始**，仓库内无 `frontend/` 目录。
...
| `GET/POST/PUT/DELETE /api/v1/{entities}` | 14 类实体 CRUD（tasks、notes、sessions…） |
...
| **D** | 笔记 / 搜索 / 回收站（FS 层 FTS 已有，部分 REST 待补） | ~75% |
| **E** | MCP / 备份 / 导出 | ~75% |
| **F** | React 19 + Next.js 15 前端 | **0%** |
| **G** | 数据迁移 / E2E | ~15% |
| **H** | 部署 / CI/CD（Docker、GHCR、DEPLOY.md） | ~70% |

**后端综合 ~88%** · **含前端完整产品 ~50%**（截至 `main` @ PR #14 合并后）。
