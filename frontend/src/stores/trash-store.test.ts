...
describe('useTrashStore QuickNote actions', () => {
  ...
  // -------- S3-Task10: version capture for trash operations --------





  it('S3T10-TS1: restoreNote captures base version as expectedVersion', async () => {
    ...
    await useTrashStore.getState().restoreNote('trash-vc-restore')



    const outbox = await db.outbox.where('entityId').equals('trash-vc-restore').first()
    ...
    const note = await db.notes.get('trash-vc-restore')

    expect(note!.version).toBe(5) // 4+1

  })



  it('S3T10-TS2: purgeNote captures base version in delete event', async () => {
    ...
    await useTrashStore.getState().purgeNote('trash-vc-purge')



    const outbox = await db.outbox.where('entityId').equals('trash-vc-purge').first()

    expect(outbox).toBeDefined()

    expect(outbox!.action).toBe('delete')

    expect(outbox!.expectedVersion).toBe(6)

  })



  it('S3T10-TS3: restoreFolder captures base version', async () => {

    await db.folders.put(makeCachedFolder({

      id: 'trash-vc-folder-restore',
    ...
    await useTrashStore.getState().restoreFolder('trash-vc-folder-restore')



    const outbox = await db.outbox.where('entityId').equals('trash-vc-folder-restore').first()
    ...
    const folder = await db.folders.get('trash-vc-folder-restore')

    expect(folder!.version).toBe(4) // 3+1

  })



  it('S3T10-TS4: purgeFolder captures base version in delete event', async () => {

    await db.folders.put(makeCachedFolder({

      id: 'trash-vc-folder-purge',

      name: 'VC Purge Folder',
    ...
    await useTrashStore.getState().purgeFolder('trash-vc-folder-purge')



    const outbox = await db.outbox.where('entityId').equals('trash-vc-folder-purge').first()

    expect(outbox).toBeDefined()

    expect(outbox!.action).toBe('delete')

    expect(outbox!.expectedVersion).toBe(8)
