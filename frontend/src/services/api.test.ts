...
describe('api.ts interceptors', () => {
  ...
  // -------- S3-Task10: Idempotency-Key interceptor --------



  it('S3T10-1: POST request gets Idempotency-Key header', async () => {
    ...
    await spaceApi.post('/tasks', { title: 'New Task' })



    expect(capturedKey).toBeDefined()

    expect(capturedKey).toHaveLength(36) // UUID format

  })



  it('S3T10-2: GET request does NOT get Idempotency-Key', async () => {
  ...
  it('S3T10-3: explicit Idempotency-Key is preserved (not overwritten)', async () => {
    ...
    await spaceApi.post('/tasks', { title: 'X' }, {

      headers: { 'Idempotency-Key': 'my-explicit-key' },
  ...
  it('S3T10-4: PUT and DELETE also get Idempotency-Key', async () => {
    ...
    spaceApi.defaults.adapter = async (config: InternalAxiosRequestConfig) => {
      ...
      const k = config.headers?.['Idempotency-Key'] as string | undefined
    ...
    await spaceApi.put('/tasks/t1', { title: 'Updated' })

    await spaceApi.delete('/tasks/t1')
  ...
  it('S3T10-5: 401 retry reuses the same Idempotency-Key', async () => {
    ...
    // Simulate setSpaceToken updating getSpaceToken
    ...
    // Mock axios.post for token reissue — must resolve with space_token

    axiosPostSpy.mockImplementation(async () => ({

      data: { space_token: 'new-token' },

    }))



    const result = await spaceApi.post('/tasks', { title: 'Retry Test' })



    // Both the original request and the 401 retry should have the same key
  ...
  it('S3T10-6: metaApi POST also gets Idempotency-Key', async () => {
    ...
    await metaApi.post('/spaces/space-1/token', {})



    expect(capturedKey).toBeDefined()

    expect(capturedKey).toHaveLength(36)
