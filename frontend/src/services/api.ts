...
import { ensureMutationIdempotencyKey } from '@/services/idempotency'
...
metaApi.interceptors.request.use((config) => {
  ...
  // S3-Task10: Authorization 注入后再调用，保证 401/CF 重试复用原 config/header
...
async function tryReissueSpaceToken(): Promise<string | null> {
  ...
  reissuePromise = promise



  // Clear single-flight lock AFTER the promise settles.



  // Using promise.finally() (not try/finally inside the IIFE) because



  // the finally block inside an async IIFE can race with vitest's microtask



  // scheduler, leaving reissuePromise stale.
