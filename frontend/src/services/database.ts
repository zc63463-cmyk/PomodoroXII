...
export class PomodoroXIDB extends Dexie {
  ...
  constructor(dbName = 'pomodoroxi') {
    ...
    // version(17): S3-Task10 — 幂等键持久化字段。

    // 不改索引（保持 outbox 现有索引定义），仅回填三个新字段。

    // 在单个 upgrade transaction 内回填，中断则整体回滚。
    ...
    this.version(17).stores({
      ...
      await tx.table('outbox').toCollection().modify((row: Record<string, unknown>) => {
        ...
        // expectedVersion / requiresVersionRebase：按 action + payload 三分支回填

        if (row.expectedVersion == null && row.requiresVersionRebase == null) {

          const action = row.action as string

          if (action === 'create') {

            // create：无 expectedVersion
            ...
            // update/delete：检查 payload 中是否有整数 version >= 2
            ...
            try {
              ...
              if (typeof payload.version === 'number' && Number.isInteger(payload.version) && payload.version >= 2) {
              ...
              // payload 不是合法 JSON → 无法确定 version

            }

            if (payloadVersion !== null) {

              row.expectedVersion = payloadVersion - 1
