...
export async function pushBatch(
  ...
  // 契约 5: 过滤 requiresVersionRebase 行（需 S4 rebase 后再推送）

  const pushable = rows.filter((r) => !r.requiresVersionRebase)
  ...
  // 契约 5: 批量幂等键作为 Idempotency-Key 头

  const batchKey = await buildBatchIdempotencyKey(pushable)
