...
describe('push-batch', () => {
  ...
  it('PB11: requiresVersionRebase rows are filtered from push', async () => {

    db = await openTestDb()

    await db.outbox.put(makeOutboxRow(1, 'task', 't1', 'create'))

    // Add a row that requires version rebase — should NOT be pushed

    await db.outbox.put({

      ...makeOutboxRow(2, 'task', 't2', 'update'),
    ...
    let postedEvents: Array<{ entity_id: string }> = []

    spaceApi.defaults.adapter = async (config: InternalAxiosRequestConfig) => {

      const body = typeof config.data === 'string' ? JSON.parse(config.data) : config.data

      postedEvents = (body as { events: Array<{ entity_id: string }> }).events

      return ok({

        applied: postedEvents.map((e) => ({ entity_type: 'task', entity_id: e.entity_id, action: 'create' })),
    ...
    // Only t1 should be pushed; t2 (requiresVersionRebase) is filtered

    expect(postedEvents).toHaveLength(1)

    expect(postedEvents[0]!.entity_id).toBe('t1')

    expect(result.clearedOutboxIds).toContain(1)

    // t2 remains in outbox

    const t2 = await db.outbox.get(2)

    expect(t2).toBeDefined()

    expect(t2!.requiresVersionRebase).toBe(true)

  })



  it('PB12: all rows requiresVersionRebase → pushBatch is a no-op', async () => {

    db = await openTestDb()

    await db.outbox.put({

      ...makeOutboxRow(1, 'task', 't1', 'update'),
  ...
  it('PB13: pushBatch sends Idempotency-Key header derived from operationIds', async () => {

    db = await openTestDb()

    await db.outbox.put(makeOutboxRow(1, 'task', 't1', 'create'))

    await db.outbox.put(makeOutboxRow(2, 'task', 't2', 'create'))
    ...
    spaceApi.defaults.adapter = async (config: InternalAxiosRequestConfig) => {
      ...
      capturedKey = config.headers?.['Idempotency-Key'] as string | undefined
    ...
    // Header should be sync-${sha256hex} = 5 + 64 = 69 chars

    expect(capturedKey).toBeDefined()

    expect(capturedKey).toHaveLength(69)

    expect(capturedKey!.startsWith('sync-')).toBe(true)

  })



  it('PB14: batch idempotency key is deterministic for same operationIds', async () => {

    const rows1 = [

      makeOutboxRow(1, 'task', 't1', 'create'),

      makeOutboxRow(2, 'task', 't2', 'create'),
    ...
    expect(key1).toHaveLength(69)

  })



  it('PB15: buildBatchIdempotencyKey rejects empty batch', async () => {
  ...
  it('PB16: buildBatchIdempotencyKey rejects rows missing operationId', async () => {

    const row = makeOutboxRow(1, 'task', 't1', 'create')

    delete (row as Partial<OutboxEvent>).operationId

    await expect(buildBatchIdempotencyKey([row])).rejects.toThrow('non-null operationId')
