...
export async function enqueueOutbox(
  ...
  if (existing.length > 0) {
    ...
    // 契约 4: keep/replace 合并绝不替换既有 operationId（latest.operationId 不变）

    // 契约 4: create 链始终为 null；update/delete 链保留最早已知 expectedVersion
    ...
    if (latest.action === 'create') {
      ...
      latest.expectedVersion = earliestExpectedVersion(
...
/** 返回两个 expectedVersion 中更早（更小）的非 null 值 */

function earliestExpectedVersion(
