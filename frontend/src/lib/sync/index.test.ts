...
describe('lib/sync/index', () => {
  ...
  it('IX2: wire 后 onPullComplete 回调 → setState + invalidate query (F1-D6b)', async () => {
    const mod = await import('./index')
    const { queryClient } = await import('@/lib/query-client')

    mod.bootstrapSyncEngine('space-1')
    const engine = mockEngineInstances[0]!
    engine.getStatus.mockReturnValueOnce('idle')
    engine.getLastSyncedAt.mockReturnValueOnce('2026-07-07T00:00:00Z')
    engine.getPendingCount.mockReturnValueOnce(5)
    engine.getConflicts.mockReturnValueOnce([])
