...
from app.registry import CATALOG, EntitySpec
...
def resolve_model(spec_or_path: Union[EntitySpec, str]) -> type:
    ...
    """
    if isinstance(spec_or_path, EntitySpec):
        return CATALOG.model_for(spec_or_path.name)
    return CATALOG.model_for(spec_or_path)
