...
This document is the **authoritative, tracked** service-level objective
contract.  It lives at `backend/SLO.md` (not under the gitignored `docs/`
tree) so that every clean checkout can evaluate the objectives against the
...
- **Action:** for a degraded space (503 from health), follow the recovery
  restore flow: run `python -m app.ops snapshot` to a verified external
  target, `python -m app.ops verify`, then restore the affected space from
  the latest verified snapshot (see the recovery operations contract).
